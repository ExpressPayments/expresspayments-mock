...
import (
	...
	"flag"
	"fmt"
	"github.com/expresspayments/expresspayments-mock/embedded"
	"net"
	"net/http"
	"os"
	"strconv"

	"github.com/expresspayments/expresspayments-mock/server"
)
...
func main() {
	...
	flag.BoolVar(&options.strictVersionCheck, "strict-version-check", false, "Errors if version sent in EP-Version doesn't match the one in OpenAPI")
	...
	fmt.Printf("expresspayments-mock %s\n", version)
	...
	// For both spec and fixtures expresspayments-mock will by default load data from
	...
	expresspaymentsSpec, err := server.LoadSpec(specBytes, options.specPath)
	if err != nil {
		abort(err.Error())
	}
	...
	stub, err := server.NewStubServer(fixtures, expresspaymentsSpec, options.strictVersionCheck, verbose)
	if err != nil {
	...
	if httpsListener != nil {
		...
		// it stays easy to distribute expresspayments-mock as a standalone binary with
...
// options is a container for the command line options passed to expresspayments-mock.
type options struct {
	fixturesPath string
