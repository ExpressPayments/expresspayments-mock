...
import (
	...
	"github.com/expresspayments/expresspayments-mock/generator/datareplacer"
...
func (g *DataGenerator) generateInternal(params *GenerateParams) (interface{}, error) {
	...
	if isBinaryResource(schema) {
		return "ExpressPayments binary response", nil
	}
	...
	// Otherwise, we'll cause bugs like https://github.com/expresspayments/expresspayments-mock/issues/447
...
// simulated new objects from expresspayments-mock all have unique IDs.
//
...
// randomID generates a ExpressPayments-like ID suitable for use identifying an object.
//
// As with the real ExpressPayments API, the general format looks like:
//
...
// simplified methodology compared to the real ExpressPayments API.
