...
import (
	...
	"github.com/expresspayments/expresspayments-mock/param"
	"github.com/expresspayments/expresspayments-mock/param/coercer"
	"github.com/expresspayments/expresspayments-mock/spec"
	"github.com/lestrrat-go/jsval"
)

// Version set in EP-Mock-Version response header
...
// generate paths with double slashes, and the real ExpressPayments API responds to
// these requests normally, so expresspayments-mock emulates that behavior.
...
// returned from ExpressPayments' API.
...
func LoadSpec(embeddedSpec []byte, specPath string) (*spec.Spec, error) {
	...
	var expresspaymentsSpec spec.Spec
	err = json.Unmarshal(data, &expresspaymentsSpec)
	if err != nil {
		return nil, fmt.Errorf("error decoding spec: %v", err)
	}

	return &expresspaymentsSpec, nil
}
...
func (s *StubServer) HandleRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !validateAuth(auth) {
		...
		expresspaymentsError := createExpressPaymentsError(typeInvalidRequestError, message)
		writeResponse(w, r, start, http.StatusUnauthorized, expresspaymentsError)
		return
	}

	// If the option `-strict-version-check` is on, any request that sends an
	// explicit `EP-Version` header must have a version that matches that
	...
	if s.strictVersionCheck {
		expresspaymentsVersion := r.Header.Get("EP-Version")
		if expresspaymentsVersion != "" && expresspaymentsVersion != s.spec.Info.Version {
			message := fmt.Sprintf(invalidExpressPaymentsVersion, expresspaymentsVersion, s.spec.Info.Version)
			expresspaymentsError := createExpressPaymentsError(typeInvalidRequestError, message)
			writeResponse(w, r, start, http.StatusBadRequest, expresspaymentsError)
			return
		}
	}
	...
	// back into response headers like the ExpressPayments API does.
	...
	if route == nil {
		...
		expresspaymentsError := createExpressPaymentsError(typeInvalidRequestError, message)
		writeResponse(w, r, start, http.StatusNotFound, expresspaymentsError)
		return
	}
	...
	// it.
	requestData, expresspaymentsError := validateAndCoerceRequest(r, route, requestData)
	if expresspaymentsError != nil {
		writeResponse(w, r, start, http.StatusBadRequest, expresspaymentsError)
		return
	}
...
func (s *StubServer) initializeRouter() error {
	...
	for path, verbs := range s.spec.Paths {
		...
		for verb, operation := range verbs {
			...
			//
			// This is all a little weird and based off of how ExpressPayments' OpenAPI
			// specification is generated which is itself based off the
			// original Rack confusion between query and body parameters
			// (because it became ossified in ExpressPayments' server implementation).
			if verb == "get" {
...
const (
	...
	invalidExpressPaymentsVersion = "Version sent in `EP-Version` header '%s' " +
		"doesn't match version in OpenAPI specification '%s' which may have " +
		"unintended consequences. This error was shown because expresspayments-mock  " +
		"was started with `-expresspayments-version-check`."
...
func createInternalServerError() *ResponseError {
	return createExpressPaymentsError(typeInvalidRequestError, internalServerError)
}

// This creates a ExpressPayments error to return in case of API errors.
func createExpressPaymentsError(errorType string, errorMessage string) *ResponseError {
...
// given operation. We don't expect any endpoint in the ExpressPayments API to have
...
func validateAndCoerceRequest(
	...
	if r.Method != http.MethodDelete && r.Method != http.MethodGet {
		...
		if contentType == "" {
			...
			fmt.Printf(message + "\n")
			return nil, createExpressPaymentsError(typeInvalidRequestError, message)
		}
...
func writeResponse(w http.ResponseWriter, r *http.Request, start time.Time, status int, data interface{}) {
	...
	w.Header().Set("EP-Mock-Version", Version)
