...
func TestStubServer_InvalidExpressPaymentsVersion(t *testing.T) {
	testBadVersion := "2006-01-01"

	headers := getDefaultHeaders()
	headers["EP-Version"] = testBadVersion
	...
	// With strictVersionCheck on, which throws the error
	{
		...
		assert.True(t, ok)
		assert.Equal(t,
			fmt.Sprintf(invalidExpressPaymentsVersion, testBadVersion, testSpecAPIVersion),
			message)
	}
...
func TestStubServer_SetsSpecialHeaders(t *testing.T) {
	...
	assert.Equal(t, http.StatusUnauthorized, resp.StatusCode)
	assert.Equal(t, Version, resp.Header.Get("EP-Mock-Version"))
...
func TestStubServer_BinaryResponse(t *testing.T) {
	...
	assert.Equal(t, "ExpressPayments binary response", string(body[:]))
}
...
func sendRequest(t *testing.T, method string, url string, params string,
	...
	fullURL := fmt.Sprintf("https://epayments.network%s", url)
